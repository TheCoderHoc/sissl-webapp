...
import { useCreateApiKeysController } from '@/modules/dashboard/controllers/usePostApiKeysControllers';
// import { GetServicesResponse } from '@/modules/dashboard/models/services';
...
const CreateApiKeyPage: React.FC = () => {
  const { createApiKey, isLoading: isCreating } = useCreateApiKeysController();
  ...
  const { data: serviceData, isLoading: isFetchingServices } = useGetServices();
 ...
 const toggleService = (serviceId: string) => {
  setSelectedServiceIds(prev => {
    const newSet = new Set(prev); // Initialize newSet
    if (newSet.has(serviceId)) {
      newSet.delete(serviceId); // Remove service if it's already selected
    } else {
      newSet.add(serviceId); // Add service if it's not selected
    }
  ...
  const allSelected = group.services.every(service => selectedServiceIds.has(service.id)); // Check if all services are selected
  setSelectedServiceIds(prev => {
    const newSet = new Set(prev); // Initialize newSet
    group.services.forEach(service => {
      if (allSelected) {
        newSet.delete(service.id); // If all are selected, remove them
      } else {
        newSet.add(service.id); // Otherwise, add all to the set
      }
    });
    return newSet; // Return the updated set
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      const payload = {
        name: apiName,
        description,
        environment: 'SandBox', // 👈 Hardcoded here
    ...
    <main className="min-h-screen bg-[#0a0a0a] text-white flex flex-col lg:flex-row">
      <section className="flex-1 p-5 flex flex-col">
        <div className="max-w-3xl w-full mx-auto flex flex-col flex-1">
          ...
          {loading || isFetchingServices || isCreating ? (
            <div className="flex justify-center pt-10"><CustomLoader /></div>
          ) : (
            <form onSubmit={handleSubmit} className="flex-1 flex flex-col justify-between">
              ...
              <div className="flex items-center pt-10 gap-4">
                ...
                >
                  {loading ? 'Creating...' : 'Create API keys'}
                </button>
              </div>
            </form>
          )}
      ...
      <aside className="hidden sm:flex lg:w-1/3 p-4 md:p-6 items-center justify-center">
        <img src={dashboardKey.src} alt="Key illustration" className="w-64 h-64 object-contain" />
      </aside>
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2
            ...
            {success ? (
              ...
              <>
                <CustomLoader />
                <p className="mt-3 text-white text-lg">Creating API key...</p>
              </>
            )}
          </Dialog.Content>
        </Dialog.Portal>
