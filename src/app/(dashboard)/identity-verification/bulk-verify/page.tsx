...
import useIdentityBulkUploadController from "@/modules/identity-verification/controllers/identityBulkUpload";
import useGetIdentityVerificationTemplate from "@/modules/identity-verification/controllers/getIdentityVerificationTemplate";

export default function VerifyUserForm() {
    const [showUploadSection, setShowUploadSection] = useState(false);
    const [selectedFile, setSelectedFile] = useState<File | null>(null);
    const { uploadIdentityBulk, isLoading } = useIdentityBulkUploadController();
    const router = useRouter();

    const downloadTemplateQuery = useGetIdentityVerificationTemplate({
        enabled: false,
    });
    ...
    const handleSubmit = async () => {
        ...
        try {
            await uploadIdentityBulk(formData);
            ...
            <div className="px-16">
                {!showUploadSection ? (
                    <DownloadArea
                        ...
                        filename="Identity_template.csv"
                    />
                ) : (
                    <UploadArea
